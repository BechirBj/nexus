...
export default function SubjectWorkspace() {
  ...
  if (!subject)
    return (
      <Layout>
        <div className="p-12 text-center">Workspace not found</div>
      </Layout>
    );
                ...
                <Link href={`/reports/${reports[reports.length - 1].id}`}>
                  <Card className="p-6 bg-card border-border/50 hover:border-primary/30 transition-colors cursor-pointer group">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-display font-semibold text-xl group-hover:text-primary transition-colors">
                        {reports[reports.length - 1].title}
                      </h4>
                      <span className="text-xs font-medium px-2 py-1 bg-secondary rounded-full uppercase tracking-wider">
                        {reports[reports.length - 1].status}
                      </span>
                    </div>
                    <p className="text-muted-foreground line-clamp-2 font-serif">
                ...
                {reports.map((report) => (
                  <Link key={report.id} href={`/reports/${report.id}`}>
                    <Card className="p-6 bg-card border-border/50 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer h-full flex flex-col group">
                      ...
                      <p className="text-muted-foreground text-sm flex-1 font-serif line-clamp-3 mb-4 leading-relaxed">
                        {report.content}
                      </p>
          ...
          <TabsContent value="timeline" className="mt-0 outline-none py-4">
            {timeline.length === 0 ? (
              <EmptyState
                icon={Clock}
                title="Quiet here"
                description="Activity will appear as you add resources and create reports."
              />
            ) : (
                ...
                {timeline.map((event) => (
                  ...
                  >
                    ...
                    <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-5 rounded-2xl border border-border/50 bg-card shadow-sm hover:shadow-md transition-shadow">
                      ...
                      <h4 className="font-display font-medium text-foreground text-base">
                        {event.title}
                      </h4>
                    </div>
                  </div>
